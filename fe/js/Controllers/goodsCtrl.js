...
define(function () {
    return {
        "ctrlName": "goodsCtrl",
        "ctrlFn": function ($scope, $modal, growl,$filter, goodsServices,$stateParams) {

            $scope.goodsId = $stateParams.goodsId;
            console.log($scope.goodsId);

            goodsServices.getGoodsDetail($scope.goodsId).then(
                function (responses) {
            ...
            //    goodsServices.getGoodsListByCategory(data).then(
            //        function (responses) {
            //            console.log(responses);
            //            if (responses.data.code!=0) {
            //                growl.addErrorMessage(responses.data.data);
